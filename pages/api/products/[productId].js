...
export default async function handler(req, res) {
  ...
  // GET HTTP
  if (req.method === "GET") {
    const { productId } = req.query; // client에서 요청 시, 파라미터가 query든, path든 Next.js에서는 무조건 req.query로 받는다!
    // ** POST, PATCH, PUT 요청 시에 필요한 데이터는 req.body로 받는다.**

    try {
      const document = await getDocumentById(client, "products", productId);
